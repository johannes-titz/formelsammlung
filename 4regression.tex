...
b_1 &= \frac{\mathrm{cov}(x,y)}{s^2_x} = r \frac{s_y}{s_x}\\
b_0 &= \bar{y}-b_1 \bar{x}\\
y_i &= \hat{y}_i + e_i
\end{align*}
...
$$s_e= \sqrt{\frac{\sum\limits_{i=1}^n(y_i-\hat{y}_i)^2}{n}}$$ 
$$\hat{\sigma}_e = s_y\cdot\sqrt{1-R^2_{\mathrm{adj}}}$$ 
...
$$s_e=\sqrt{\frac{\sum\limits_{i=1}^n(y_i-\hat{y}_i)^2}{n}}$$
$$\hat{\sigma}_e = s_y \sqrt{1-R^2_{\mathrm{adj}}}$$

$$\hat{\sigma}_e = \sqrt{\frac{\sum\limits_{i=1}^n(y_i-\hat{y}_i)^2}{n-k-1}} = \hat{\sigma}_y \cdot \sqrt{1-R^2_{\mathrm{korr}}}$$

Korrigiertes $R^2$ (für die Prädiktorenzahl $k$ korrigiert)
$$R^2_{\mathrm{korr}}=1-(1-R^2)\frac{n-1}{n-k-1}$$
