...
\subsection*{Pearson-Residuum}

$$r_{ij}=\frac{f_{b,ij}-f_{e,ij}}{\sqrt{f_{e,ij}}}$$

$\sum r_{ij}^2=\chi^2$

Faustregel: $|r_{ij}|>2$ deutet auf eine auffällige Zelle hin.

\subsection*{$\chi^2$-Effektgrößen}
...
Phi-Koeffizient (nur für $2\times2$-Tabellen):
\[
...
Umrechnung von $V$ in $w$:
\[
w=V\sqrt{\min(k,m)-1}
\]

\textbf{Hinweis:} Für $2\times2$-Kontingenztafeln gilt

\[
w=\phi=V=\sqrt{\frac{\chi^2}{N}}.
\]

Die drei Effektgrößen sind in diesem Fall identisch und unterscheiden sich erst bei größeren Tabellen.
...
Approximation über Normalverteilung bei großen Stichproben:
